...
class CUnmanagedDAFFHandle {
  public:
	inline CUnmanagedDAFFHandle() : pReader(nullptr) {};

	inline ~CUnmanagedDAFFHandle() { delete pReader; };
...
void NativeDAFFContentIRGetRecordCoords(DAFFContentIR* pDAFFContent, const int iIndex, double* pdAzimuth,
										double* pdElevation)
{
	if (pDAFFContent == nullptr)
		return;
...
bool NativeDAFFContentIRGetRecordData(DAFFContentIR* pDAFFContent, const int iIndex, const int iChannel,
									  float* pcSamples)
{
	if (pDAFFContent == nullptr)
		return false;
...
void NativeDAFFContentMSGetRecordCoords(DAFFContentMS* pDAFFContent, const int iIndex, double* pdAzimuth,
										double* pdElevation)
{
	if (pDAFFContent == nullptr)
		return;
...
bool NativeDAFFContentMSGetRecordData(DAFFContentMS* pDAFFContent, const int iIndex, const int iChannel,
									  float* pcSamples)
{
	if (pDAFFContent == nullptr)
		return false;
