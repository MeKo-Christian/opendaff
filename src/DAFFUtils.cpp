...
std::string DAFFUtils::Float2StrNice(float f, int precision, bool showpos, int leadingzeros)
{
	...
	if (leadingzeros > 0)
		ss << std::setw(leadingzeros + precision + 1) << std::setfill('0');
	if (showpos)
		ss << fabs(f);
	else
		ss << f;
...
std::string DAFFUtils::StrDirection(int iView, double dAngle1, double dAngle2, int precision, int leadingzeros)
{
	...
	switch (iView) {
	case DAFF_DATA_VIEW:
		ss << std::fixed << "( A" << Double2StrNice(dAngle1, precision, false, leadingzeros) << "\xF8, B"
		   << Double2StrNice(dAngle2, precision, false, leadingzeros) << "\xF8 )";
		return ss.str();
		break;

	case DAFF_OBJECT_VIEW:
		ss << std::fixed << std::showpos << "( P" << Double2StrNice(dAngle1, precision, true, leadingzeros) << "\xF8, T"
		   << Double2StrNice(dAngle2, precision, true, leadingzeros) << "\xF8 )";
		return ss.str();
		break;
...
std::string DAFFUtils::StrContentType(int iContentType)
{
	...
	case DAFF_DFT_SPECTRUM:
		return "Discrete Fourier-spectrum";
	default:
		return "Invalid";
	}
}
