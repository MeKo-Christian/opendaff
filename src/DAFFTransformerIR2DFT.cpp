...
class DAFFContentDFTRealization : public DAFFContentDFT {
  public:
	...
	inline double getSamplerate() const { return m_pInputContent->getSamplerate(); };

	inline double getFrequencyBandwidth() const { return getSamplerate() / (double)getTransformSize(); };

	inline float getOverallMagnitudeMaximum() const { return m_pParent->getOverallMagnitudeMaximum(); };
	...
	inline DAFFReader* getParent() const { return m_pInputContent->getParent(); };
	...
	inline void getNearestNeighbour(int iView, float fAngle1, float fAngle2, int& iRecordIndex,
	...
	inline void transformAnglesD2O(float fAlpha, float fBeta, float& fAzimuth, float& fElevation) const
	{
		m_pInputContent->transformAnglesD2O(fAlpha, fBeta, fAzimuth, fElevation);
	};

	inline void transformAnglesO2D(float fAzimuth, float fElevation, float& fAlpha, float& fBeta) const
	{
...
void DAFFTransformerIR2DFT::transform()
{
	...
	if (m_pOutputContent) {
		...
		fftwf_plan oFFTPlan =
			fftwf_plan_dft_r2c_1d(iFilterLength, pfInputData, reinterpret_cast<fftwf_complex*>(m_pfBuf), FFTW_ESTIMATE);
...
int DAFFTransformerIR2DFT::getDFTCoeff(int iRecordIndex, int iChannel, int iDFTCoeff, float& fReal, float& fImag) const
{
	...
	if ((iRecordIndex < 0) || (iRecordIndex >= iRecords))
		return -1;
	if ((iChannel < 0) || (iChannel >= iChannels))
		return -1;
	if ((iDFTCoeff < 0) || (iDFTCoeff > m_iNumDFTCoeffs))
		return -1;
