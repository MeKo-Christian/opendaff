...
int exportIR(DAFFContentIR* pContentIR, int iRecordIndex, const std::string& sFilename, int iView, bool bQuiet,
			 bool bVerbose)
{
	...
	if (!bQuiet) {
		printf("Exported impulse response to \"%s\" (%i samples | %i channels | %s Hz | %s)\n", sFilename.c_str(),
			   iFilterLength, iChannels, DAFFUtils::Float2StrNice((float)pContentIR->getSamplerate(), 1, false).c_str(),
...
int exportMS(DAFFContentMS* pContentMS, const std::string& sFilename, int iRecordIndex, float fAngle1, float fAngle2,
			 int iView, bool bQuiet, bool bVerbose)
{
	...
	if (fprintf(file, "%s",
				convertMS2dat(pContentMS, iRecordIndex, fAngle1, fAngle2, iView, bQuiet, bVerbose).c_str()) < 0) {
		fprintf(stderr, "Error: Failed to write output file \"%s\"\n", sFilename.c_str());
		fclose(file);
		return 255;
	}
...
int exportPS(DAFFContentPS* pContentPS, const std::string& sFilename, int iRecordIndex, float fAngle1, float fAngle2,
			 int iView, bool bQuiet, bool bVerbose)
{
	FILE* file = fopen(sFilename.c_str(), "w");
	...
	if (fprintf(file, "%s",
				convertPS2dat(pContentPS, iRecordIndex, fAngle1, fAngle2, iView, bQuiet, bVerbose).c_str()) < 0) {
		fprintf(stderr, "Error: Failed to write output file \"%s\"\n", sFilename.c_str());
		fclose(file);
		return 255;
	}
...
int exportMPS(DAFFContentMPS* pContent, const std::string& sFilename, int iRecordIndex, float fAngle1, float fAngle2,
			  int iView, bool bQuiet, bool bVerbose)
{
	FILE* file = fopen(sFilename.c_str(), "w");
	...
	if (fprintf(file, "%s", convertMPS2dat(pContent, iRecordIndex, fAngle1, fAngle2, iView, bQuiet, bVerbose).c_str()) <
		0) {
		fprintf(stderr, "Error: Failed to write output file \"%s\"\n", sFilename.c_str());
		fclose(file);
		return 255;
...
int exportDFT(DAFFContentDFT* pContent, const std::string& sFilename, int iRecordIndex, float fAngle1, float fAngle2,
			  int iView, bool bQuiet, bool bVerbose)
{
	FILE* file = fopen(sFilename.c_str(), "w");
	...
	if (fprintf(file, "%s", convertDFT2dat(pContent, iRecordIndex, fAngle1, fAngle2, iView, bQuiet, bVerbose).c_str()) <
		0) {
		fprintf(stderr, "Error: Failed to write output file \"%s\"\n", sFilename.c_str());
		fclose(file);
		return 255;
...
std::string convertMS2dat(DAFFContentMS* pContentMS, int iRecordIndex, float fAngle1, float fAngle2, int iView,
						  bool bQuiet, bool bVerbose)
{
	std::stringstream ss;

	ss << "# Support frequencies [Hz]" << std::endl << "freqs = ";
	...
	for (int iIndex = iStart; iIndex <= iEnd; iIndex++) {
		...
		ssLine << "# ";
		if (bReverseAngles)
			ssLine << cAngle2ViewFlag << DAFFUtils::Double2StrNice(fBeta, 3, bObjectView, 3) << " " << cAngle1ViewFlag
		<< DAFFUtils::Double2StrNice(fAlpha, 3, bObjectView, 3); else ssLine << cAngle1ViewFlag <<
		DAFFUtils::Double2StrNice(fAlpha, 3, bObjectView, 3) << " " << cAngle2ViewFlag <<
		DAFFUtils::Double2StrNice(fBeta, 3, bObjectView, 3); ss << ssLine.str() << std::endl;
		*/

		if (bVerbose)
			printf("Exporting magnitude spectrum for direction %s\n",
		...
		for (int iChannel = 0; iChannel < pContentMS->getProperties()->getNumberOfChannels(); iChannel++) {
			...
			ss << "direction: " << DAFFUtils::StrDirectionCompact(iView, fAngle1, fAngle2)
			   << ", channel: " << (iChannel + 1) << " = ";
...
std::string convertPS2dat(DAFFContentPS* pContentPS, int iRecordIndex, float fAngle1, float fAngle2, int iView,
						  bool bQuiet, bool bVerbose)
{
	std::stringstream ss;
	ss << "# Support frequencies [Hz]" << std::endl << "freqs = ";
	...
	for (int iIndex = iStart; iIndex <= iEnd; iIndex++) {
		...
		*/
		...
		if (bVerbose)
			printf("Exporting phase spectrum for direction %s\n",
				   DAFFUtils::StrDirection(iView, fAngle1, fAngle2).c_str());
...
std::string convertMPS2dat(DAFFContentMPS* pContent, int iRecordIndex, float fAngle1, float fAngle2, int iView,
						   bool bQuiet, bool bVerbose)
{
	std::stringstream ss;
	ss << "# Support frequencies [Hz]" << std::endl << "freqs = ";
	...
	for (int iIndex = iStart; iIndex <= iEnd; iIndex++) {
		...
		if (bVerbose)
			printf("Exporting magnitude-phase spectrum for direction %s\n",
				   DAFFUtils::StrDirection(iView, fAngle1, fAngle2).c_str());
		...
		for (int iChannel = 0; iChannel < pContent->getProperties()->getNumberOfChannels(); iChannel++) {
			...
			for (int i = 0; i < 2 * pContent->getNumFrequencies(); i++) {
				if (i > 0)
					ss << ", ";
				ss << "(" << DAFFUtils::Float2StrNice(pfDest[2 * i], 9, false) << ", "
				   << DAFFUtils::Float2StrNice(pfDest[2 * i + 1], 9, false) << ")";
			}
			ss << std::endl;
...
std::string convertDFT2dat(DAFFContentDFT* pContent, int iRecordIndex, float fAngle1, float fAngle2, int iView,
						   bool bQuiet, bool bVerbose)
{
	std::stringstream ss;
	...
	for (int iIndex = iStart; iIndex <= iEnd; iIndex++) {
		...
		if (bVerbose)
			printf("Exporting DFT spectrum for direction %s\n",
				   DAFFUtils::StrDirection(iView, fAngle1, fAngle2).c_str());
