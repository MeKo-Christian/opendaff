...
#else
	DWORD atts;
	return !sPath.empty() && ((atts = GetFileAttributesA(sPath.c_str())) != 0xFFFFFFFF) &&
...
int main_info(int argc, char* argv[])
{
	...
	case DAFF_MAGNITUDE_SPECTRUM:
		...
		for (size_t i = 0; i < pContentMS->getFrequencies().size(); i++) {
			...
			if (i < (pContentMS->getFrequencies().size() - 1))
				printf(", ");
		}
		printf(" Hz\n");
		break;
	...
	for (std::vector<std::string>::const_iterator cit = vsKeyList.begin(); cit != vsKeyList.end(); ++cit) {
		std::string sType = std::string("[") +
							DAFFUtils::StrMetadataKeyType(g_pDAFFReader->getMetadata()->getKeyType(*cit)) +
							std::string("]");
		printf("%-10s %s: %s\n", sType.c_str(), (*cit).c_str(),
			   g_pDAFFReader->getMetadata()->getKeyString(*cit).c_str());
	}

	if (vsKeyList.empty())
		printf("< No metadata defined >\n");
	printf("\n");
...
int main_dump(int argc, char* argv[])
{
	bool bForce = false, bObjectView = false, bQuiet = false, bReverseAngles = false, bVerbose = false;
	...
	if (bVerbose)
		printf("Using output directory directory \"%s\"\n", sTargetDirectory.c_str());
	...
	case DAFF_IMPULSE_RESPONSE:
		if (bVerbose)
			printf("Detected impulse response content type\n");
		pContentIR = dynamic_cast<DAFFContentIR*>(g_pDAFFReader->getContent());
		...
		for (iRecordIndex = 0; iRecordIndex < g_pDAFFReader->getProperties()->getNumberOfRecords(); iRecordIndex++) {
			...
			if (bReverseAngles)
				ssOutFileName << cAngle2ViewFlag << DAFFUtils::Double2StrNice(fAngle2, 3, bObjectView, 3) << "_"
							  << cAngle1ViewFlag << DAFFUtils::Double2StrNice(fAngle1, 3, bObjectView, 3);
			else
				ssOutFileName << cAngle1ViewFlag << DAFFUtils::Double2StrNice(fAngle1, 3, bObjectView, 3) << "_"
							  << cAngle2ViewFlag << DAFFUtils::Double2StrNice(fAngle2, 3, bObjectView, 3);
	...
	case DAFF_MAGNITUDE_SPECTRUM:
		if (bVerbose)
			printf("Detected magnitude spectrum content type\n");
		pContentMS = dynamic_cast<DAFFContentMS*>(g_pDAFFReader->getContent());
		...
		if (!bQuiet)
			printf("Exported magnitude spectrum to file \"%s\"\n", sOutFileName.c_str());
	...
	case DAFF_PHASE_SPECTRUM:
		if (bVerbose)
			printf("Detected phase spectrum content type\n");
		...
		if (!bQuiet)
			printf("Exported phase spectrum to file \"%s\"\n", sOutFileName.c_str());
	...
	case DAFF_MAGNITUDE_PHASE_SPECTRUM:
		if (bVerbose)
			printf("Detected magnitude-phase spectrum content type\n");
		...
		if (!bQuiet)
			printf("Exported magnitude-phase spectrum to file \"%s\"\n", sOutFileName.c_str());
	...
	case DAFF_DFT_SPECTRUM:
		if (bVerbose)
			printf("Detected discrete fourier spectrum content type\n");
		pContentDFT = dynamic_cast<DAFFContentDFT*>(g_pDAFFReader->getContent());
		...
		if (!bQuiet)
			printf("Exported discrete fourier spectrum to file \"%s\"\n", sOutFileName.c_str());
...
int main_query(int argc, char* argv[])
{
	bool bConsoleOutput = false, bForce = false, bReverseAngles = false, bQuiet = false, bVerbose = false;
	std::string sFilePrefix, sFilename, sTargetDirectory;
	...
	int iView = -1;
	if ((cAngle1ViewFlag == 'P') && (cAngle2ViewFlag == 'T'))
		iView = DAFF_OBJECT_VIEW;
	if ((cAngle1ViewFlag == 'A') && (cAngle2ViewFlag == 'B'))
		iView = DAFF_DATA_VIEW;
	if (iView == -1) {
		fprintf(stderr, "Error: Could not determine whether the requested coordinates are of data or of object view "
						"perspective\n.");
	...
	if (bVerbose)
		iView == DAFF_DATA_VIEW ? printf("Using data view\n") : printf("Using object view\n");
	...
	case DAFF_IMPULSE_RESPONSE:
		...
		if (!bQuiet && bOutOfBounds)
			printf("Requested angle %s is out of bounds\n",
				   DAFFUtils::StrDirection(iView, fAngle1, fAngle2, 3, 3).c_str());
		if (bVerbose)
			printf("Nearest neighbouring record index is %i\n", iRecordIndex);

		// Generate filename
		if (sOutFileName.length() == 0) {
			if (bReverseAngles) {
				if (iView == DAFF_DATA_VIEW)
					ssOutFileName << cAngle2ViewFlag << DAFFUtils::Double2StrNice(fAngle2_projected, 3, false, 3) << "_"
								  << cAngle1ViewFlag << DAFFUtils::Double2StrNice(fAngle1_projected, 3, false, 3);
				else
					ssOutFileName << cAngle2ViewFlag << DAFFUtils::Double2StrNice(fAngle2_projected, 3, true, 3) << "_"
								  << cAngle1ViewFlag << DAFFUtils::Double2StrNice(fAngle1_projected, 3, true, 3);
			} else {
				if (iView == DAFF_DATA_VIEW)
					ssOutFileName << cAngle1ViewFlag << DAFFUtils::Double2StrNice(fAngle1_projected, 3, false, 3) << "_"
								  << cAngle2ViewFlag << DAFFUtils::Double2StrNice(fAngle2_projected, 3, false, 3);
				else
					ssOutFileName << cAngle1ViewFlag << DAFFUtils::Double2StrNice(fAngle1_projected, 3, true, 3) << "_"
								  << cAngle2ViewFlag << DAFFUtils::Double2StrNice(fAngle2_projected, 3, true, 3);
			}
			ssOutFileName << ".wav";
			sOutFileName = ssOutFileName.str();
		}
	...
	case DAFF_MAGNITUDE_SPECTRUM:
		...
		// CONSOLE output
		if (bConsoleOutput) {
			printf("\n%s", convertMS2dat(pContentMS, iRecordIndex, fAngle1_projected, fAngle2_projected, iView, bQuiet,
										 bVerbose)
							   .c_str());
			return 0;
		}
		...
		// Write TEXTFILE
		iResult = exportMS(pContentMS, sOutFileName, iRecordIndex, fAngle1_projected, fAngle2_projected, iView, bQuiet,
						   bVerbose);
		if (iResult != 0) {
			tidyup();
			return iResult;
	...
	case DAFF_PHASE_SPECTRUM:
		...
		// CONSOLE output
		if (bConsoleOutput) {
			printf("\n%s", convertPS2dat(pContentPS, iRecordIndex, fAngle1_projected, fAngle2_projected, iView, bQuiet,
										 bVerbose)
							   .c_str());
			return 0;
		}
		...
		// Write TEXTFILE
		iResult = exportPS(pContentPS, sOutFileName, iRecordIndex, fAngle1_projected, fAngle2_projected, iView, bQuiet,
						   bVerbose);
		if (iResult != 0) {
			tidyup();
			return iResult;
	...
	case DAFF_MAGNITUDE_PHASE_SPECTRUM:
		...
		// CONSOLE output
		if (bConsoleOutput) {
			printf("\n%s", convertMPS2dat(pContentMPS, iRecordIndex, fAngle1_projected, fAngle2_projected, iView,
										  bQuiet, bVerbose)
							   .c_str());
			return 0;
		}
		...
		// Write TEXTFILE
		iResult = exportMPS(pContentMPS, sOutFileName, iRecordIndex, fAngle1_projected, fAngle2_projected, iView,
							bQuiet, bVerbose);
		if (iResult != 0) {
			tidyup();
			return iResult;
	...
	case DAFF_DFT_SPECTRUM:
		if (bVerbose)
			printf("Detected DFT spectrum content type\n");
		pContentDFT = dynamic_cast<DAFFContentDFT*>(g_pDAFFReader->getContent());
		...
		// CONSOLE output
		if (bConsoleOutput) {
			printf("\n%s", convertDFT2dat(pContentDFT, iRecordIndex, fAngle1_projected, fAngle2_projected, iView,
										  bQuiet, bVerbose)
							   .c_str());
			return 0;
		}
		...
		// Write TEXTFILE
		iResult = exportDFT(pContentDFT, sOutFileName, iRecordIndex, fAngle1_projected, fAngle2_projected, iView,
							bQuiet, bVerbose);
		if (iResult != 0) {
			tidyup();
			return iResult;
		...
		if (!bQuiet)
			printf("Exported DFT spectrum to file \"%s\"\n", sOutFileName.c_str());
