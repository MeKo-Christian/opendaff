...
class DAFFContentModel : public QAbstractTableModel {
  ...
  public:
	inline DAFFContentModel(QObject* pParent) : QAbstractTableModel(pParent) {};

	inline int rowCount(const QModelIndex&) const = 0;

	inline int columnCount(const QModelIndex&) const { return 2; };
...
class QDAFFContentIRModel : public DAFFContentModel {
  public:
	inline QDAFFContentIRModel(QObject* pParent, DAFFContentIR* pContent)
		: DAFFContentModel(pParent), m_pContent(pContent)
	{
	}

	inline int rowCount(const QModelIndex&) const { return 5; };
	...
	inline QVariant data(const QModelIndex& index, int iRole = Qt::DisplayRole) const
	{
		if (iRole == Qt::DisplayRole) {
			switch (index.row()) {
			case 0:
				return (index.column() == 0 ? QString("Filter length")
											: QString::number(m_pContent->getFilterLength()));
			case 1:
				return (index.column() == 0 ? QString("Maximum effective filter length")
											: QString::number(m_pContent->getMaxEffectiveFilterLength()));
			case 2:
				return (index.column() == 0 ? QString("Minimum effective filter offset")
											: QString::number(m_pContent->getMinEffectiveFilterOffset()));
			case 3:
				return (index.column() == 0 ? QString("Overall peak value")
											: QString::number(m_pContent->getOverallPeak()));
			case 4:
				return (index.column() == 0 ? QString("Sampling rate")
											: QString("%1 Hz").arg(m_pContent->getSamplerate()));
			}
		}
...
class QDAFFContentMSModel : public DAFFContentModel {
  public:
	...
	inline int rowCount(const QModelIndex&) const { return 3; };
	...
	inline QVariant data(const QModelIndex& index, int iRole = Qt::DisplayRole) const
	{
		if (iRole == Qt::DisplayRole) {
			switch (index.row()) {
			case 0:
				return (index.column() == 0 ? QString("Number of frequencies")
											: QString::number(m_pContent->getNumFrequencies()));
			case 1:
				return (index.column() == 0 ? QString("Frequency range")
											: QString("%1 Hz - %2 Hz")
												  .arg(m_pContent->getFrequencies().front())
												  .arg(m_pContent->getFrequencies().back()));
			case 2:
				return (index.column() == 0 ? QString("Overall maximum magnitude")
											: QString::number(m_pContent->getOverallMagnitudeMaximum()));
			}
		}
...
class QDAFFContentPSModel : public DAFFContentModel {
  public:
	...
	inline int rowCount(const QModelIndex&) const { return 2; };
...
class QDAFFContentDFTModel : public DAFFContentModel {
  public:
	...
	inline int rowCount(const QModelIndex&) const { return 6; };
	...
	inline QVariant data(const QModelIndex& index, int iRole = Qt::DisplayRole) const
	{
		if (iRole == Qt::DisplayRole) {
			switch (index.row()) {
			case 0:
				return (index.column() == 0 ? QString("Frequency bandwidth")
											: QString::number(m_pContent->getFrequencyBandwidth()));
			case 1:
				return (index.column() == 0 ? QString("DFT transform size")
											: QString::number(m_pContent->getTransformSize()));
			case 2:
				return (index.column() == 0 ? QString("Symmetrical DFT spectrum")
											: QString(m_pContent->isSymmetric() ? "yes" : "no"));
			case 3:
				return (index.column() == 0 ? QString("Sampling rate")
											: QString("%1 Hz").arg(m_pContent->getSamplerate()));
			case 4:
				return (index.column() == 0 ? QString("Number of DFT coefficiants")
											: QString("%1 Hz").arg(m_pContent->getNumDFTCoeffs()));
			case 5:
				return (index.column() == 0 ? QString("Overall maximum magnitude")
											: QString::number(m_pContent->getOverallMagnitudeMaximum()));
			}
...
class QDAFFContentTableView : public QTableView {
  ...
  public:
	inline QDAFFContentTableView(QWidget* parent = Q_NULLPTR) : QTableView(parent), m_pModel(NULL) {}
  ...
  public slots:
	...
	inline void ReadDAFF(const DAFFReader* pReader)
	{
		...
		case DAFF_IMPULSE_RESPONSE: {
			// No const pointer because one of the methods uses lazy initialization (calculates member value on first
			// call)
