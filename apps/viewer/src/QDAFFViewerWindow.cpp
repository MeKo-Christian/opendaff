...
QDAFFViewerWindow::QDAFFViewerWindow(QWidget* parent, QString sPath)
	: QMainWindow(parent), ui(new Ui::DAFFViewer), m_pDAFFReader(DAFFReader::create()),
	  m_qSettings("ITA", "DAFFViewer"), m_iShowChannel(0), m_iShowFrequencyIndex(0), m_dShowTimeSample(0.0f),
	  m_dShowAlphaDeg(0.0f), m_dShowBetaDeg(90.0f), m_dShowPhiDeg(0.0f), m_dShowThetaDeg(0.0f),
	  m_dPhiThetaIncrementDeg(1.0f)
{
	...
	connect(this, SIGNAL(SignalReadDAFF(const DAFFReader*)), ui->tableView_Properties,
			SLOT(ReadDAFF(const DAFFReader*)));
	connect(this, SIGNAL(SignalReadDAFF(const DAFFReader*)), ui->tableView_Content, SLOT(ReadDAFF(const DAFFReader*)));
	connect(this, SIGNAL(SignalReadDAFF(const DAFFReader*)), ui->graphicsView_2DDAFFPlot,
			SLOT(ReadDAFF(const DAFFReader*)));
	connect(this, SIGNAL(SignalReadDAFF(const DAFFReader*)), ui->DAFF3DPlot_VTKWidget,
			SLOT(ReadDAFF(const DAFFReader*)));
	...
	connect(this, SIGNAL(SignalFrequencyIndexChanged(int)), ui->graphicsView_2DDAFFPlot,
			SLOT(ChangeFrequencyIndex(int)));
	...
	// 3D conns
	connect(this, SIGNAL(SignalExportScreenshotPNG(QString)), ui->DAFF3DPlot_VTKWidget,
			SLOT(ExportScreenshotPNG(QString)));


	// Menu settings
	ui->action3DSphericalShowArrows->setChecked(
		m_qSettings.value("Settings/3DPlot/Spherical/ShowArrows", true).toBool());
	...
	ui->action3DSphericalShowEquator->setChecked(
		m_qSettings.value("Settings/3DPlot/Spherical/ShowEquator", true).toBool());
	ui->action3DSphericalShowMeridians->setChecked(
		m_qSettings.value("Settings/3DPlot/Spherical/ShowMeridians", false).toBool());
	ui->action3DSphericalUsePhaseColorMap->setChecked(
		m_qSettings.value("Settings/3DPlot/Spherical/UsePhaseColorMap", false).toBool());
	ui->action3DUseLogScale->setChecked(m_qSettings.value("Settings/3DPlot/LogScale", true).toBool());
	ui->action3DSphericalNormalize->setChecked(
		m_qSettings.value("Settings/3DPlot/Spherical/Normalize", false).toBool());
	ui->action3DSphericalNormalizeFrequenciesIndividually->setChecked(
		m_qSettings.value("Settings/3DPlot/Spherical/NormalizeFreqsIndividually", false).toBool());
	...
	ui->DAFF3DPlot_VTKWidget->SetNormalizeFrequenciesIndividually(
		ui->action3DSphericalNormalizeFrequenciesIndividually->isChecked());
	ui->DAFF3DPlot_VTKWidget->SetCarpetPlotWarp(ui->action3DCarpetShowWarp->isChecked());

	// Other

	ui->DAFFStatusBar->showMessage(
		"No DAFF file loaded. Shortcut for opening DAFF files: 'o' as in 'open'. 'Esc' will exit DAFFViewer.");

	if (!ui->DAFF3DPlot_VTKWidget->SupportsOpenGL())
		ui->DAFFStatusBar->showMessage(
			"Your graphics card does not support OpenGL 4 shader. 3D plot window will be disabled.");
...
QDAFFViewerWindow::~QDAFFViewerWindow()
{
	...
	m_qSettings.setValue("Settings/3DPlot/Spherical/UsePhaseColorMap",
						 ui->action3DSphericalUsePhaseColorMap->isChecked());
	...
	m_qSettings.setValue("Settings/3DPlot/Spherical/NormalizeFreqsIndividually",
						 ui->action3DSphericalNormalizeFrequenciesIndividually->isChecked());
...
void QDAFFViewerWindow::OpenDAFFFile(QString sPath, bool bQuiet)
{
	...
	if ((iError = m_pDAFFReader->openFile(oPassedFile.absoluteFilePath().toStdString())) != DAFF_NO_ERROR) {
		QErrorMessage qe;
		QString sError = "Could not open requested file '" + oPassedFile.fileName() +
						 "': " + QString(DAFFUtils::StrError(iError).c_str());
...
void QDAFFViewerWindow::on_actionAboutOpenDAFF_triggered()
{
	QString msg;
	msg +=
		QString("OpenDAFF is a free and open source software package for directional audio content like directivities "
				"of sound sources (e.g. loudspeakers, musical instruments) and sound receivers (e.g. microphones, "
				"head-related transfer functions, HRIRs/HRTFs). OpenDAFF enables a simple exchange, representation and "
				"efficient storage of such directional content in form of a single DAFF file (*.DAFF)\t\n");
...
void QDAFFViewerWindow::LoadContent(const DAFFContent* pContent)
{
	...
	case DAFF_IMPULSE_RESPONSE: {
		...
		for (int i = 0; i < pC->getFilterLength(); i++) {
			...
			ui->comboBox_FrequencySelector->addItem(QString::number(i / pC->getSamplerate() * 1000.0f * 1000.0f) +
													" us");
		}
...
void QDAFFViewerWindow::ChangeRecordIndex(int iRecordIndex)
{
	...
	if (m_pDAFFReader->isFileOpened()) {
		...
		m_pDAFFReader->getContent()->getNearestNeighbour(DAFF_OBJECT_VIEW, m_dShowPhiDeg, m_dShowThetaDeg,
														 iRecordIndexCurrentObjectView);
...
void QDAFFViewerWindow::ChangePhiAndTheta(double dPhiDeg, double dThetaDeg)
{
	...
	if (m_pDAFFReader->isFileOpened()) {
		int iRecordIndex;
		bool bOutOfBounds;
		m_pDAFFReader->getContent()->getNearestNeighbour(DAFF_OBJECT_VIEW, m_dShowPhiDeg, m_dShowThetaDeg, iRecordIndex,
														 bOutOfBounds);
		ChangeRecordIndex(iRecordIndex);
...
void QDAFFViewerWindow::on_actionExport2DPlot_triggered()
{
	...
	if (dialog.exec()) {
		...
		if (dialog.GetExportType() == QDAFFDialogExport2DPlot::EXPORT_TYPE_PNG) {
			ui->graphicsView_2DDAFFPlot->ExportImagePNG(sExportFilePath, fScalingFactor, bIncludeAllChannels,
														bShowDataPointDots);
			...
			ui->graphicsView_2DDAFFPlot->ExportImageSVG(sExportFilePath, fScalingFactor, bIncludeAllChannels,
														bShowDataPointDots);
			ui->DAFFStatusBar->showMessage("Exported plot to " + sExportFilePath);
		} else {
			QErrorMessage qe;
...
void QDAFFViewerWindow::on_actionExport3DPlotImageSeries_triggered()
{
	...
	if (dialog.exec()) {
		...
		oAnimation.bChannels =
			dialog.GetAnimationChannelIndices(oAnimation.iChannelIdxStart, oAnimation.iChannelIdxEnd);
