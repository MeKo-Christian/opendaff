...
QDAFFVTKWidget::QDAFFVTKWidget(QWidget* parent)
	: QVTKWidget(parent), m_pRenderer(NULL), m_pSGRootNode(NULL), m_pSCA(NULL), m_pDAFFContentBalloon(NULL),
	  m_pCCA(NULL), m_pDAFFContentCarpet(NULL), m_pSDI(NULL), m_bBalloonPhaseColor(false), m_bCarpetWarp(true),
	  m_iCarpetScaling(DAFFViz::CarpetPlot::SCALING_LINEAR), m_iBalloonScaling(DAFFViz::BalloonPlot::SCALING_DECIBEL)
{
	...
	if (SupportsOpenGL()) {
		...
		vtkSmartPointer<vtkInteractorStyleTerrain> pCustomInteractorStyle =
...
void QDAFFVTKWidget::ExportScrenshotSeriesPNG(QString sFileBasePath, QString sFileBaseName, const CAnimation& oA)
{
	...
	for (int i = 0; i < oA.iNumFrames; i++) {
		...
		if (oA.bChannels) {
			int iIdx = oA.iChannelIdxStart +
					   int(std::round((oA.iChannelIdxEnd - oA.iChannelIdxStart) * double(i) / double(oA.iNumFrames)));
			ChangeChannelIndex(iIdx);
		}

		if (oA.bFrequencies) {
			int iIdx = oA.iFreqIdxStart +
					   int(std::round((oA.iFreqIdxEnd - oA.iFreqIdxStart) * double(i) / double(oA.iNumFrames)));
