...
class QInfoBox : public QGraphicsTextItem {
  public:
	...
	inline QRectF boundingRect() const { return QGraphicsTextItem::boundingRect().adjusted(-2, -2, +2, +2); }
...
class QGraphicsPoint : public QGraphicsItem {
  public:
	...
	inline ~QGraphicsPoint() { delete m_pBox; }
  ...
  protected:
	inline void hoverEnterEvent(QGraphicsSceneHoverEvent*)
	{
		...
		m_pBox->setPos(pos().x() + boundingRect().right(),
					   pos().y() + boundingRect().bottom() + m_pBox->boundingRect().bottom());
		scene()->addItem(m_pBox);
	}

	inline void hoverLeaveEvent(QGraphicsSceneHoverEvent*) { scene()->removeItem(m_pBox); }
...
class QDAFF2DPlot : public QGraphicsView {
  ...
  public:
	...
	void ExportImagePNG(const QString& sTargetFilePath, float fScaling = 1.0f, bool bShowAllChannels = false,
						bool bShowDataPointDots = true);
	void ExportImageSVG(const QString& sTargetFilePath, float fScaling = 1.0f, bool bShowAllChannels = false,
						bool bShowDataPointDots = true);
  ...
  private:
	...
	// drawing
	const int m_iDrawLimit =
		500;  //!< determines at which number of data points the algorithm starts drawing only visible points
