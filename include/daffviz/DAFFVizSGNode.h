...
class DAFF_API SGNode {
  public:
	...
	//! Sets the orientation of the node assembly based on yaw, pitch and roll angles around own coordinate axis (angles
	//! in [&deg;])
	/**
	...
	//! Orientation getter of the node assembly based on yaw, pitch and roll angles around own coordinate axis (angles
	//! in [&deg;])
	/**
	 * Will first rotate around -z axis (roll), then x (pitch), then y (yaw)
	 *
