...
class DAFF_API DAFFUtils {
  public:
	 ...
	 */
	static std::string StrDirection(int iView, double dAngle1Deg, double dAngle2Deg, int precision = 9,
									int leadingzeros = 3);
	 ...
	 * Format for object view directions [P => phi, T => theta]: P+123.1&deg; T-10.167&deg;
	 */
	static std::string StrDirectionCompact(int iView, double dAngle1Deg, double dAngle2Deg, int precision = 9,
										   int leadingzeros = 3);
	 ...
	 */
	static void NormalizeDirection(int iView, float fAngle1DegIn, float fAngle2DegIn, float& fAngle1DegOut,
								   float& fAngle2DegOut);
	...
	inline static float grad2radf(float phi) { return phi * DAFF::PI_F / 180.0f; };

	inline static float rad2gradf(float phi) { return phi * 180.0f / DAFF::PI_F; };

	inline static double grad2rad(double phi) { return phi * DAFF::PI_D / 180.0f; };

	inline static double rad2grad(double phi) { return phi * 180.0f / DAFF::PI_D; };
