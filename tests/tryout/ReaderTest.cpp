...
int main()
{
	...
	// check impulse response
	{
		...
		for (i = 0; i < num; i++)  // i=recordindex
		{
			for (int j = 0; j < 2; j++) {
				...
				if ((fabs(h[0]) > EPSILON) || (fabs(h[1]) > EPSILON) || (fabs(h[2]) > EPSILON) ||
					(fabs(h[3]) > EPSILON) || (fabs(h[4]) > EPSILON) || (fabs(h[5] - 1) > EPSILON) ||
					(fabs(h[6] * 360 - alpha) > EPSILON) || (fabs(h[7] * 360 - beta) > EPSILON)) {
					cerr << "Coeffs do not match: " << h[0] << " " << h[1] << " " << h[2] << " " << h[3] << " " << h[4]
						 << " " << h[5] << " " << h[6] << " " << h[7] << endl;
	...
	// check magnitude spectrum
	{
		...
		for (int i = 0; i < num; i++)  // i=recordindex
		{
			...
			x->getRecordCoords(i, DAFF_DATA_VIEW, alpha, beta);
			if ((fabs(h[0]) > EPSILON) || (fabs(h[1]) > EPSILON) || (fabs(h[2]) > EPSILON) || (fabs(h[3]) > EPSILON) ||
				(fabs(h[4] - 1) > EPSILON) || (fabs(h[5] * 360 - alpha) > EPSILON) ||
